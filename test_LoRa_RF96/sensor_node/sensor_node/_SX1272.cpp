...
uint8_t _SX1272::ON()
{
  ...
  // Init SPI (add a delay before serial communications - adviced)
  delay(100);
	SPI.begin();
...
/*
 ...
 Parameters:
   ...
   //modified by dani (added another argm)
*/
int8_t _SX1272::setPower(char p, char q)
{
 
  byte RegPaDacReg=0x4D;    //Check datasheet 
  ...
  ///////////////////
   ...
   if (q=='x') {
    ...
    //Based on datasheet to set max output power (check page 88)
    if (q=='X') {
        // normally value = 0x0F;
...
/*
 ...
 Parameters:
   ...
   0 to 14 dBm.
*/
int8_t _SX1272::setPowerNum(uint8_t powt)
{
  byte st0;
  int8_t state = 2;
  byte value = 0x00;
	...
	{ 
		...
		// Wait until the packet is sent (TX Done flag) or the timeout expires
  		...
  		while ((bitRead(value, 3) == 0) && (millis() - previous < wait))
  		{
  			...
  			if( millis() < previous ) // Condition to avoid an overflow (DO NOT REMOVE)
  			{
  				previous = millis();
  			}
  		}
...
void _SX1272::setSleepMode()
{
    ...
    { // LoRa mode
        ...
        writeRegister(REG_OP_MODE, LORA_SLEEP_MODE);  // Stdby mode to write in registers
    }
