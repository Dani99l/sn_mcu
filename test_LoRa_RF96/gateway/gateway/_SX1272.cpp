...
uint8_t _SX1272::ON()
{
	...
	// Powering the module
    pinMode(SOCKET0, OUTPUT);
    digitalWrite(SOCKET0, HIGH);
    
    // update SPI flag
//	SPI.isSocket0 = true;
  
	//Configure the MISO, MOSI, CS, SPCR.
  SPI.pins(SCK, MISO, MOSI, SOCKET0);
  delay(100);
	SPI.begin();
  delay(100);
...
//	SPI.close();
	...
	// Powering the module
  digitalWrite(SOCKET0, OUTPUT);
  digitalWrite(SOCKET0, LOW);
...
*/
byte _SX1272::readRegister(byte address)
{
    byte value = 0x00;

    // Select SX slave
    digitalWrite(SOCKET0_SELECT, LOW);
    ...
    digitalWrite(SOCKET0_SELECT, HIGH);
...
*/
void _SX1272::writeRegister(byte address, byte data)
{
    // Select SX slave
    digitalWrite(SOCKET0_SELECT, LOW);
...
uint8_t _SX1272::getMode()
{
	...
	{								//		_bandwidth
	  ...
	  Serial.printHex(_bandwidth);
	  Serial.println();
	  Serial.print(F("\t Coding Rate: "));
	  Serial.printHex(_codingRate);
	  Serial.println();
	  Serial.print(F("\t Spreading Factor: "));
	  Serial.printHex(_spreadingFactor);
	  Serial.println(F(" ##"));
	  Serial.println();
	#endif
...
*/
int8_t _SX1272::setMode(uint8_t mode)
{
	int8_t state = 2;
	byte st0;
	byte config1 = 0x00;
...
*/
boolean	_SX1272::isSF(uint8_t spr)
{
  #if (SX1272_debug_mode > 1)
	  Serial.println();
...
*/
uint8_t	_SX1272::setSF(uint8_t spr)
{
	byte st0;
	int8_t state = 2;
	byte config1;
...
*/
boolean	_SX1272::isBW(uint16_t band)
{
  #if (SX1272_debug_mode > 1)
	  Serial.println();
...
*/
int8_t	_SX1272::setBW(uint16_t band)
{
  byte st0;
  int8_t state = 2;
  byte config1;
...
*/
boolean	_SX1272::isCR(uint8_t cod)
{
  #if (SX1272_debug_mode > 1)
	  Serial.println();
...
*/
int8_t	_SX1272::setCR(uint8_t cod)
{
  byte st0;
  int8_t state = 2;
  byte config1;
...
*/
boolean	_SX1272::isChannel(uint32_t ch)
{
  #if (SX1272_debug_mode > 1)
	  Serial.println();
...
*/
int8_t _SX1272::setChannel(uint32_t ch)
{
  byte st0;
  int8_t state = 2;
  unsigned int freq3;
...
*/
int8_t _SX1272::setPower(char p)
{
  byte st0;
  int8_t state = 2;
  byte value = 0x00;
...
/*
 ...
 Parameters:
   ...
   0 to 14 dBm.
*/
int8_t _SX1272::setPowerNum(uint8_t pow)
{
  byte st0;
  int8_t state = 2;
  byte value = 0x00;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::getPreambleLength()
{
	int8_t state = 2;
	uint8_t p_length;
	...
	if( _modem == LORA )
  	{ // LORA mode
		...
		_preamblelength = _preamblelength + (p_length & 0xFFFF);
			...
			Serial.printHex(_preamblelength);
			Serial.print(F(" ##"));
			Serial.println();
		#endif
	}
...
*/
uint8_t _SX1272::setPreambleLength(uint16_t l)
{
	byte st0;
	uint8_t p_length;
	int8_t state = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::getPayloadLength()
{
	uint8_t state = 2;
	...
	{ // FSK mode
		...
		Serial.printHex(_payloadlength);
		Serial.println(F(" ##"));
		Serial.println();
	#endif
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = -1 --> Forbidden command for this protocol
*/
int8_t _SX1272::setPacketLength()
{
	uint16_t length;
...
*/
int8_t _SX1272::setPacketLength(uint8_t l)
{
	byte st0;
	byte value = 0x00;
	int8_t state = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::getNodeAddress()
{
	byte st0 = 0;
	uint8_t state = 2;
...
*/
int8_t _SX1272::setNodeAddress(uint8_t addr)
{
	byte st0;
	byte value;
	uint8_t state = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = -1 -->
*/
uint8_t _SX1272::setRetries(uint8_t ret)
{
	uint8_t state = 2;
...
*/
int8_t _SX1272::setMaxCurrent(uint8_t rate)
{
	int8_t state = 2;
	byte st0;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::truncPayload(uint16_t length16)
{
	uint8_t state = 2;
...
uint8_t _SX1272::setACK()
{
	...
	if( state == 0 )
	{
		...
		writeRegister(REG_FIFO, ACK.data[0]);	// Writing the ACK in FIFO
			...
			Serial.printHex(ACK.dst);			 	// Printing destination
			Serial.print("|");
			Serial.printHex(ACK.src);			 	// Printing source
			Serial.print("|");
			Serial.printHex(ACK.packnum);			// Printing ACK number
			Serial.print("|");
			Serial.printHex(ACK.length);				// Printing ACK length
			Serial.print("|");
			Serial.printHex(ACK.data[0]);			// Printing ACK payload
			Serial.println(F(" ##"));
			Serial.println();
		#endif
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::receivePacketMAXTimeout()
{
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::receivePacketTimeout()
{
	setTimeout();
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::receivePacketTimeout(uint32_t wait)
{
	uint8_t state = 2;
	uint8_t state_f = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::receivePacketMAXTimeoutACK()
{
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::receivePacketTimeoutACK()
{
	setTimeout();
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::receivePacketTimeoutACK(uint32_t wait)
{
	uint8_t state = 2;
	uint8_t state_f = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::receiveAll(uint32_t wait)
{
	uint8_t state = 2;
	byte config1;
...
*/
boolean	_SX1272::availableData(uint32_t wait)
{
	byte value;
	byte header = 0;
	boolean forme = false;
	...
	{ 
		...
		if( (_destination == _nodeAddress) || (_destination == BROADCAST_0) || _destination==1 )
		{ // LoRa or FSK mode
			forme = true;
			#if (SX1272_debug_mode > 0)
...
/*
 ...
 Returns:  Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::getPacketMAXTimeout()
{
...
/*
 ...
 Parameters:
   wait: time to wait while there is no a valid header received.
*/
int8_t _SX1272::getPacket(uint32_t wait)
{
	uint8_t state = 2;
	uint8_t state_f = 2;
	byte value = 0x00;
	...
	{		
		...
		else
		{
			...
			// Print the packet if debug_mode
				...
				Serial.printHex(packet_received.dst);			 	// Printing destination
				Serial.print("|");
				Serial.printHex(packet_received.src);			 	// Printing source
				Serial.print("|");
				Serial.printHex(packet_received.packnum);			// Printing packet number
				Serial.print("|");
				Serial.printHex(packet_received.length);			// Printing packet length
				Serial.print("|");
				for(unsigned int i = 0; i < _payloadlength; i++)
				{
					Serial.printHex(packet_received.data[i]);		// Printing payload
					Serial.print("|");
				}
				Serial.printHex(packet_received.retry);			// Printing number retry
				Serial.println(F(" ##"));
				Serial.println();
			#endif
			state_f = 0;
...
*/
int8_t _SX1272::setDestination(uint8_t dest)
{
	int8_t state = 2;
...
/*
 ...
 Returns: Float that determines the time-on-air
*/
float _SX1272::timeOnAir( uint16_t payloadlength )
{
	float BW;
	float DE = 0;
...
/*
 ...
 Returns:  Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::setPayload(char *payload)
{
	uint8_t state = 2;
	uint8_t state_f = 2;
	uint16_t length16;
...
/*
 ...
 Returns:  Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::setPayload(uint8_t *payload)
{
	uint8_t state = 2;
...
/*
 ...
 Returns:  Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::setPacket(uint8_t dest, char *payload)
{
	int8_t state = 2;
	byte st0;
	...
	if( state == 0 )
	{
		...
		state = 0;
			...
			Serial.printHex(packet_sent.dst);			 	// Printing destination
			Serial.print("|");
			Serial.printHex(packet_sent.src);			 	// Printing source
			Serial.print("|");
			Serial.printHex(packet_sent.packnum);			// Printing packet number
			Serial.print("|");
			Serial.printHex(packet_sent.length);			// Printing packet length
			Serial.print("|");
			for( uint16_t i = 0; i < _payloadlength; i++)
			{
				Serial.printHex(packet_sent.data[i]);		// Printing payload
				Serial.print("|");
			}
			Serial.printHex(packet_sent.retry);			// Printing retry number
			Serial.println(F(" ##"));
		#endif
	}
	writeRegister(REG_OP_MODE, st0);	// Getting back to previous status
...
/*
 ...
 Returns:  Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::setPacket(uint8_t dest, uint8_t *payload)
{
	int8_t state = 2;
	byte st0;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendWithMAXTimeout()
{
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendWithTimeout()
{
	setTimeout();
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendWithTimeout(uint32_t wait)
{
	uint8_t state = 2;
	byte value = 0x00;
	unsigned long previous;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketMAXTimeout(uint8_t dest, char *payload)
{
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketMAXTimeout(	uint8_t dest,  
											uint8_t *payload, 
											uint16_t length16)
{
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketTimeout(uint8_t dest, char *payload)
{
	uint8_t state = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketTimeout(	uint8_t dest, 
										uint8_t *payload, 
										uint16_t length16)
{
	uint8_t state = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketTimeout(uint8_t dest, char *payload, uint32_t wait)
{
	uint8_t state = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketMAXTimeoutACK(uint8_t dest, char *payload)
{
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketMAXTimeoutACK(uint8_t dest, 
											uint8_t *payload, 
											uint16_t length16)
{
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketTimeoutACK(uint8_t dest, char *payload)
{
	uint8_t state = 2;
	uint8_t state_f = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketTimeoutACK(	uint8_t dest, 
											uint8_t *payload, 
											uint16_t length16)
{
	uint8_t state = 2;
...
/*
 ...
 Parameters:
   wait: time to wait while there is no a valid header received.
*/
uint8_t _SX1272::getACK(uint32_t wait)
{
	uint8_t state = 2;
	byte value = 0x00;
	unsigned long previous;
				...
				if( ACK.packnum == packet_sent.packnum )
				{
					if( ACK.length == 0 )
					{
						if( ACK.data[0] == CORRECT_PACKET )
						{
							...
							Serial.printHex(ACK.dst);
							Serial.print("|");
							Serial.printHex(ACK.src);
							Serial.print("|");
							Serial.printHex(ACK.packnum);
							Serial.print("|");
							Serial.printHex(ACK.length);
							Serial.print("|");
							Serial.printHex(ACK.data[0]);
							Serial.println(F(" ##"));
							Serial.println();
							#endif
						}
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketMAXTimeoutACKRetries(	uint8_t dest, 
													char  *payload)
{
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketTimeoutACKRetries(uint8_t dest, char *payload)
{
	uint8_t state = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t _SX1272::sendPacketTimeoutACKRetries(uint8_t dest, 
												uint8_t *payload, 
												uint16_t length16)
{
	uint8_t state = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
void _SX1272::showFramefromPacket()
{	
	uint8_t length = 0;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
void _SX1272::showReceivedPacket()
{	
	uint8_t length = 0;
...
*/
uint8_t _SX1272::showFramefromPacketMeshlium()
{
	uint8_t state = 2;
	uint8_t length = 0;
    length = packet_received.length - OFFSET_PAYLOADLENGTH;
...
/*
 ...
 Returns: Integer that determines if there has been any error   
   ...
   state = 0  --> The command has been executed with no errors
//*/
//uint8_t _SX1272::setRTCfromMeshlium()
//{
//	uint8_t length = 0;
//	byte status;
//	char buffer[100];
...
//	{
//		#if (SX1272_debug_mode > 1)  
//			Serial.println(F("packet sent ERROR"));
//		#endif 
//		return 1;
//	}
//		
...
//	{
//		#if (SX1272_debug_mode > 1)
//		Serial.println(F("Show packet received: "));
//		showReceivedPacket();
//		#endif
//
//		// get payload length
...
//		{
//			#if (SX1272_debug_mode > 1) 
//				Serial.println(F("ERROR: buffer not enough for length"));      
...
//	{
//		#if (SX1272_debug_mode > 1) 
//			Serial.print(F("Receiving packet TIMEOUT, state "));
